...
if (isset($_POST['delete'])) {
    $location = Location::getLocationWithId($_POST['id']);
    $_SESSION['deleted_location'] = $location;
    Location::deleteLocation($_POST['id']);
    
    // Set delete message
    $_SESSION['delete_message'] = "Location deleted successfully!";
...
if (isset($_POST['undo_delete'])) {
    if (isset($_SESSION['deleted_location'])) {
        $deletedLocation = $_SESSION['deleted_location'];
        $location = new Location();
        $location->setLocation($deletedLocation['name']);
        $location->save();

        unset($_SESSION['deleted_location']);
        $_SESSION['success_message'] = "Location restored successfully!";
    ...
    <div class="flex w-full relative">
        ...
        <div class="ml-72 px-14 py-10 flex-1">
            ...
            <div class="flex justify-between w-full pb-10">
                ...
                <button class="flex items-center justify-center gap-2.5 px-4 uppercase rounded bg-yellow-400 location__btn">
                    ...
                    <span class="font-semibold whitespace-nowrap">Create location</span>
                </button>
            </div>
            <div class="grid grid-cols-2 gap-x-12 gap-y-10">
                <?php foreach ($locations as $location) : ?>
                    <a href="location.php?id=<?php echo $location['id']; ?>&mode=read">
                        <article class="p-6 border-gray-200 border-2 rounded-lg bg-white shadow-md">

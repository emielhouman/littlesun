...
// Fetch the assigned tasks for the selected member if a member is selected
$assignedTasks = [];
if (isset($_GET['id'])) {
    $assignedTasks = Task::getTasksByUserId($_GET['id']);
}

// Fetch member and location details if member ID and mode are set
if (isset($_GET['id']) && isset($_GET['mode'])) {
    $member = Member::getMemberWithId($_GET['id']);
    $memberLocation = Location::getLocationWithId($member['location_id']);
    $locations = Location::getAll();

    // Handle task assignment form submission
    if (isset($_POST['assign_tasks'])) {
        $task = new Task();
        $task->setUserId($_GET['id']);
        $task->assignTasks(isset($_POST['tasks']) ? $_POST['tasks'] : []);
        $assignedTasks = Task::getTasksByUserId($_GET['id']); // Refresh assigned tasks after update
    }

    // Handle member update form submission
    if ($_GET['mode'] === "edit" && !empty($_POST)) {
        ...
        $member->setLocationId($_POST['location_id']);
        ...
        header("Location: members.php?id=" . $_GET['id'] . "&mode=read");
        exit;
    ...
    <div class="flex w-screen relative">
            ...
            <div class="w-2/5 p-12 bg-white rounded-lg relative">
                ...
                <div class="flex flex-col gap-4">
                    <?php foreach ($locations as $location) : ?>
                        <?php $locationMember = Member::getMemberWithLocationId($location['id']); ?>
                        <button class="flex items-center justify-between p-3.5 rounded shadow border-2 border-gray-300 <?php echo ($locationMember) ? "bg-gray-50" : "bg-white" ?> select__btn" type="button" data-id="<?php echo $location['id'] ?>">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col items-start">
                                    ...
                                    <span class="font-semibold text-sm text-yellow-400 uppercase"><?php echo ($locationMember) ? $locationMember['firstname'] . " " . $locationMember['lastname'] : "No HUB Member" ?></span>
                                </div>
                            </div>
        ...
        <div class="ml-72 px-14 py-10 flex-1">
            <div class="flex justify-between mb-12">
                <h2 class="font-extrabold text-4xl">HUB member: <span class="font-normal"><?php echo $member['firstname'] . " " . $member['lastname'] ?></span></h2>
                <a href="members.php?id=<?php echo $member['id'] ?>&mode=<?php echo ($_GET['mode'] === "edit" ? "read" : "edit") ?>" class="h-12 flex items-center justify-center gap-2 px-4 font-semibold uppercase rounded cursor-pointer <?php echo ($_GET['mode'] === "edit" ? "bg-gray-200 border-gray-200" : "bg-yellow-400 border-yellow-400") ?>">
            ...
            <form class="w-full grid grid-cols-2 gap-x-14 gap-y-6 p-16 border-2 rounded-lg shadow-md" action="" method="post">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input class="w-full h-12 px-2.5 text-lg opacity-100 rounded border-2 border-gray-300 <?php echo ($_GET['mode'] === "edit" ? "bg-white" : "bg-gray-100") ?>" id="firstname" name="firstname" type="text" value="<?php echo $member["firstname"] ?>" <?php echo ($_GET['mode'] === "edit" ? "" : "disabled") ?>>
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="font-semibold text-lg" for="lastname">Lastname</label>
                    <input class="w-full h-12 px-2.5 text-lg opacity-100 rounded border-2 border-gray-300 <?php echo ($_GET['mode'] === "edit" ? "bg-white" : "bg-gray-100") ?>" id="lastname" name="lastname" type="text" value="<?php echo $member["lastname"] ?>" <?php echo ($_GET['mode'] === "edit" ? "" : "disabled") ?>>
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="font-semibold text-lg" for="email">Email</label>
                    <input class="w-full h-12 px-2.5 text-lg opacity-100 rounded border-2 border-gray-300 <?php echo ($_GET['mode'] === "edit" ? "bg-white" : "bg-gray-100") ?>" id="email" name="email" type="email" value="<?php echo $member["email"] ?>" <?php echo ($_GET['mode'] === "edit" ? "" : "disabled") ?>>
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="font-semibold text-lg" for="location">HUB Location:</label>
                    <input class="location__input" id="location" type="hidden" name="location_id" value="<?php echo (!empty($memberLocation)) ? $memberLocation['id'] : "0" ?>">
                    <button class="w-full h-12 flex items-center justify-between px-2.5 rounded border-2 border-gray-300 bg-gray-100"<?php echo ($_GET['mode'] === "edit" ? "bg-white" : "bg-gray-100") ?> location__select" type="button" disabled <?php echo ($_GET['mode'] === "edit" ? "" : "disabled") ?>>
                        <?php if (!empty($memberLocation)) : ?>
                            <span class="text-lg"><?php echo $memberLocation['name'] ?></span>
                ...
                <div class="pt-8 col-span-2">
                    <input class="w-full h-12 font-bold uppercase rounded border-2 cursor-pointer <?php echo ($_GET['mode'] === "edit" ? "bg-yellow-400 border-yellow-400" : "bg-gray-200 border-gray-200") ?>" name="assign_tasks" type="submit" value="<?php echo ($_GET['mode'] === "edit" ? "Update member" : "Go back") ?>">
                </div>
            </form>
        </div>
    </div>
